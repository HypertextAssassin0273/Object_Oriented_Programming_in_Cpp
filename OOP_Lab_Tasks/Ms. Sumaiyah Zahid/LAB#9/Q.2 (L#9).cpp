...
public:
	Prime_Number(unsigned int Prime_No=2):Prime_No(Prime_No){} //i.e. defaults to 1st prime (2)
	
	unsigned int Get_Prime_No()const{ return Prime_No; }
	void Set_Prime_No(unsigned int Prime_No){ this->Prime_No=Prime_No; }
	
	friend Prime_Number operator++(Prime_Number&); //i.e. pre-increment
	friend Prime_Number operator--(Prime_Number&); //i.e. pre-decrement
	friend Prime_Number operator++(Prime_Number&,int); //i.e. post-increment
	friend Prime_Number operator--(Prime_Number&,int); //i.e. post-decrement
	
	friend Whole_Number;
};

Prime_Number operator++(Prime_Number& obj){ //i.e. pre-increment
         obj.Prime_No=obj.Find_Prime_by_Index(obj.Prime_No,1);
         return obj;
}
Prime_Number operator--(Prime_Number& obj){ //i.e. pre-decrement
         obj.Prime_No=obj.Find_Prime_by_Index(obj.Prime_No,0);
         return obj;
}
Prime_Number operator++(Prime_Number& obj,int){ //i.e. post-increment
		Prime_Number temp;
         temp=obj.Prime_No;
         obj.Prime_No=obj.Find_Prime_by_Index(obj.Prime_No,1);
         return temp;
}
Prime_Number operator--(Prime_Number& obj,int){ //i.e. post-decrement
		Prime_Number temp;
         temp=obj.Prime_No;
         obj.Prime_No=obj.Find_Prime_by_Index(obj.Prime_No,0);
         return temp;
...
class Whole_Number{
public:
	void Display_Prime_No_Limit()const{
		cout<<"Prime Numbers Limit: "<<Prime_Number::Prime_No_Limit<<"\n\n";
	}
	void Display_Prime_Number(const Prime_Number obj,unsigned short obj_no)const{
		cout<<"Obj "<<obj_no<<": "<<obj.Prime_No<<endl;
	}
};
...
int main(){
	Whole_Number obj;
	obj.Display_Prime_No_Limit();
	Prime_Number obj1(11),obj2(17);
	cout<<"Initial values:-\n";
	obj.Display_Prime_Number(obj1,1);
	obj.Display_Prime_Number(obj2,2);

	obj2=++obj1;
	cout<<"\nPre-increment of Obj1 & assigning Obj1 to Obj2:-\n";
