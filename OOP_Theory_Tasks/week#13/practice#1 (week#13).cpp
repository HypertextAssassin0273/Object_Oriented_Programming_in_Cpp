...
public:
	...
	Friend(string name,int age,float height):ID(++obj_counter),name(name),age(age),height(height){}
...
int main(){
	{//i.e. outer local scope, destroys the list alongwith objs
		...
		{//i.e. inner local scope, destroys all objs
			...
			Friend::Total_objs(); //Note: vector.push_back() calls dtors of objs if vector-size is reallocted
		...
		for(vector <Friend>::iterator it=vector_list.begin();it!=vector_list.end();++it)
			it->display_info();
		{1,2,3,4} {4,2,3,4} {4,2,3,1} {4,3,2,1}
		cout<<"\nAfter Reverse:-"<<endl;		
